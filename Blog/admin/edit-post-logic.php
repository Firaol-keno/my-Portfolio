...
// make sure edit post button was clicked
  ...
  if(isset($_POST['submit'])){
    ...
    // set iis_featured to 0 if it was unchecked
    $is_featured = $is_featured == 1 ?: 0;

     //check and validate input values
    if(!$title) {
      $_SESSION['edit-post'] = "Couldn't update title post. Invalid form data on edit post page";
      $re = "error";
    } elseif(!$category_id) {
      $_SESSION['edit-post'] = "Couldn't update post category . Invalid form data on edit post page";
      $re = "error";
    } elseif(!$body) {
      $_SESSION['edit-post'] = "Couldn't update post body. Invalid form data on edit post page";
      $re = "error";
    } else {
    ...
    $extension = end($extension);
      ...
      if(in_array($extension, $allowed_files)) {
        // make sure image size is not large (2mb+)
          if($thumbnail['size'] < 2000000) {
              //upload the thumbnail
              move_uploaded_file($thumbnail_tmp_name, $thumbnail_destination_path);
          } else {
              $_SESSION['edit-post'] = "Counld not update post. Thumbnail size too big should be less than 2mb";
          }
      } else{
        $_SESSION['edit-post'] = "Counld not update post. Thumbnail should be png, jpg or jpeg";
      }
      }
    } 
   ...
   $thumbnail_to_insert = $thumbnail_name ?? $previous_thumbnail_name;
    ...
    // now insert the updated posts into a database
